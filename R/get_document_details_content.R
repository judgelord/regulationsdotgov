...
# this helper that does the work for each document
# keeping just the content, not the full results, in memory while the main loop runs
get_document_details_content <- function(id,
                                        lastModifiedDate,
                                        api_keys){
  
  
  path <- make_path_document_details(id, sample(api_keys, 1) )
  ...
  while( result$status_code == 429 ){
    ...
    # try again
    path <- make_path_document_details(id, api_key)
    result <- httr::GET(path)
